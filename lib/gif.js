     ...
     return new Promise((resolve, reject) => {
          ...
          Axios({
               ...
               Axios({
                    ...
                    const $ = cheerio.load(data)
                    const result = 'https:' + $('div${prefix}output > p.outfile > video > source').attr('src')
                    resolve({
                         status: true,
...
async function igDownloader(Link) {
	...
	await axios(`https://downloadgram.org/`, {
		...
		let url = $('${prefix}downloadBox').find('a').attr('href');
		await axios(Link, {
			method: "GET",
			data: null,
			headers: {
...
async function TiktokDownloader(Url) {
	return new Promise (async (resolve, reject) => {
		await axios.request({
			...
			const token = $('${prefix}token').attr('value')
			axios({
				...
				const result = {
					status: res.status,
					result: {
						nowatermark: ch('${prefix}results-list > div:nth-child(2)').find('div.download > a').attr('href'),
						watermark: ch('${prefix}results-list > div:nth-child(3)').find('div.download > a').attr('href'),
						audio: ch('${prefix}results-list > div:nth-child(4)').find(' div.download > a').attr('href')
					}
				}
				resolve(result)
			}).catch(reject)
